...
TELEGRAM_BOT_TOKEN = '7774369692:AAGJqlYXFNBj7nD8ApI8EKLfaMLVQplzOdw'
...
async def approve(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Lawde madarchod bhosdike You need admin permission to use this command.*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if str(chat_id) not in approved_ids and user_id not in approved_ids:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ bhosdike madarchod bahenchod you need permission to use this bot.*", parse_mode='Markdown')
        return

    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="* Lawde wait 3 to 5 minutes for the next attack.*", parse_mode='Markdown')
        return
